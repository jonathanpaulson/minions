/**
  * Rand.scala
  * A simple class for random number generation, independent of the scala or java built-in random. Unlike them, high-period and high-quality.
  */
...
class Rand private (seeds: Array[Long]) {
  ...
  def shuffleInplace[T](arr: Array[T]): Unit = {
  ...
  def shuffle[T: ClassTag](list: List[T]): List[T] = {
...
class XorShift1024Mult(initS: Array[Long]) extends LongGen {
  ...
  private val s: Array[Long] = initS.clone
